...
These are deliberately not committed. `MYSQL_PASSWORD` is used by the MySQL
local root and bootstrap application account. Use a unique value for local
development and never use a production secret in `.env`.
