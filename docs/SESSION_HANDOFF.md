...
Cache invalidation validation used a separate isolated Docker Compose project, `m12cacheverify`, with MySQL on `127.0.0.1:3308` and Redis on `127.0.0.1:6381`. The build-tagged integration test dynamically discovered a seeded product with SKUs and passed both cases: all product/SKU cache keys were immediately removed and persisted tasks reached `DONE`; when a wrapper failed the first immediate Redis `Delete`, that key remained while its task was `PENDING`, then the bounded real worker removed it and all tasks reached `DONE`. Cleanup verification reported zero cache invalidation tasks, the seed product version restored to `1`, and Redis database size `0`.
