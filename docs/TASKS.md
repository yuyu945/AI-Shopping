...
- 当前阶段：M1.1 已完成；M1.2 的商品读链路与用户认证链路已完成，延迟二次缓存删除与失败重试待实现；M2-M6 尚未开始。
...
- 状态标记：`[ ]` 未开始，`[-]` 进行中，`[x]` 已完成，`[!]` 受阻。M1.1 已完成；M1.2 仅余缓存失效 scheduler；M2-M6 尚未开始。
