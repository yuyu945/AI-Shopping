...
5. `product-service` 在一个 `catalog_db` transaction 内幂等消费确认事件，同时把预留置为 `CONFIRMED` 和写入消费记录；任一步失败均 rollback，最多 5 次指数退避重试。畸形事件或重试耗尽时，consumer 将原始字节以 `raw_event_base64` 连同稳定 `reason` 发布到 `inventory.reservation.confirm.deadletter`；仅收到 producer ack 后才提交原消息 offset，DLQ 发布失败不得提交。预留过期时先查询 order 的结算状态，`PAID` 则确认，未支付或已取消才置为 `RELEASED` 并返还库存；查询失败保留预留并退避。
