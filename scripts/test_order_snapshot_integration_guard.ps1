...
foreach ($required in @("AI_SHOPPING_ORDER_SNAPSHOT_INTEGRATION -ne '1'", "`$project = 'm21orderverify'", 'COMPOSE_PROJECT_NAME = $project', "AI_SHOPPING_INTEGRATION_ISOLATED = `$project", "AI_SHOPPING_INTEGRATION_RUN_ID = `$runID", "AI_SHOPPING_MYSQL_DSN =", 'finally {', 'docker compose -f $composeFile down -v --remove-orphans', 'Remove-Item "Env:$name"')) {
