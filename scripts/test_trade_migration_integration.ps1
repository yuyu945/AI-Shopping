...
try {
    ...
    function Apply-AndAssert([string]$caseName) {
        ...
        if (($result -join ',') -ne '3,4,1,2,5,2,1,3') { throw "$caseName migration assertions failed: $($result -join ',')." }
    }
    Invoke-RootSQL -Sql 'DROP TABLE IF EXISTS order_items; DROP TABLE IF EXISTS cart_items; DROP TABLE IF EXISTS orders; DROP TABLE IF EXISTS carts; DROP TABLE IF EXISTS schema_migrations; DROP TABLE IF EXISTS catalog_db.inventory_reservations;' | Out-Null
